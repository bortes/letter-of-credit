 ...
 * @author bortes
 */
function StepSendTo(props) {
    const { items, handleStepChange, selected } = props;

    const [ currentSendTo, setCurrentSendTo ] = React.useState(selected);
    ...
    const handleChange = (event, item) => {
        ...
        setCurrentSendTo(newValue);
        handleStepChange(newValue);
    };

    return (
        <>
            <Typography component="h4" variant="h6">Definida o porto de atracagem.</Typography>

            <div className="row mt-4">
                {items.map((item, index) => (
                    <div className="col-md-2" key={item.port_id}>
                        <Card variant="outlined" className={classes.card}>
                            <CardActionArea className={currentSendTo.includes(item.port_id) ? '' : 'data-disabled'} onClick={e => handleChange(e, item)}>
